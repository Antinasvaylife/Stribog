/**
*	Файл содержит арифметические и криптографические функции
*/ 

#include "crypto.h"

/**
*	@brief Определение S-блока по ГОСТ-34.11
*/
static const uint8_t gost3411_Sbox[SBOX_SIZE] =
{
	...
	64, 134, 172, 29, 247, 48, 55, 107, 228, 136, 217, 231, 137,225,27,131,73, 76, 63, 248, 254, 141,
	83, 170, 144, 202, 216, 133, 97, 32, 113, 103, 164, 45, 43,9,91,203, 155, 37, 208, 190, 229, 108,
	82, 89, 166, 116, 210, 230, 244, 180, 192, 209, 102, 175, 194, 57, 75, 99, 182,
};

/**
*	@brief Определение перестановки Тау байтов блока по ГОСТ-34.11
*/
static const uint8_t gost3411_Perm[BLOCK_BYTE_SIZE] =
{
...
/**
*	@brief Определение матрицы, на которую умножаем 64-битные куски блока 
*	@details aka "линейное преобразование по ГОСТ-34.11"
*/
static const uint64_t gost3411_Matrix[QWORD_BIT_SIZE] =
{
...
/*
*	@brief Определение раундовых констант по ГОСТ-34.11
*	@details В отличие от стандарта, здесь числа представлены в обратном порядке байт
*/
static const uint512_t gost3411_iter_consts[COUNT_OF_ROUNDS] =
{
	{
...
/**
*	@brief Функция осуществляет сложение 512-битных чисел без учёта остатка
*	@details aka "сложение в кольце Z/(2^512)"
*/
static void add512(uint512_t* result, const uint512_t* a, const uint512_t* b)
{
	int i;
	uint16_t overflowed_sum = 0;
	for (i = 0; i < BLOCK_BYTE_SIZE; i++)
	{
		overflowed_sum = a->u8[i] + b->u8[i] + (overflowed_sum >> 8);
		result->u8[i] = overflowed_sum & 0xff;
	}
}

/**
*	@brief Функция осуществляет XOR 512-битных чисел
*/
static void xor512(uint512_t* result, const uint512_t* a, const uint512_t* b)
{
	int i = 0;
	for (i = 0; i < BLOCK_QWORD_SIZE; i++)
	{
...
/**
*	@brief Функция осуществляет перестановку байтов блока
*/
static void perm512(uint512_t* state) 
{
	uint512_t temp = { 0 };
	int i = 0;
	for (i = 0; i < BLOCK_BYTE_SIZE; i++)
	{
		temp.u8[i] = state->u8[gost3411_Perm[i]];
	}
...
/**
*	@brief Функция осуществляет нелинейную замену байтов блока (S-box)
*/
static void sbox512(uint512_t* state) 
{
	uint512_t temp = { 0 };
	int i = 0;
	for (i = 0; i < BLOCK_BYTE_SIZE; i++)
	{
		temp.u8[i] = gost3411_Sbox[state->u8[i]];
	}
...
/*
*	@brief Функция осуществляет линейное преобразование по ГОСТ-34.11
*/
static void mul_matrix512(uint512_t* state) 
{
	int i = 0, j = 0;
	uint64_t current = 0, temp = 0;
	for (i = 0; i < BLOCK_QWORD_SIZE; i++)
	{
		current = state->u64[i];
		for (j = 0; j < QWORD_BIT_SIZE; j++)
		{
			if ((current >> j) & 1) 
			{
				temp ^= gost3411_Matrix[QWORD_BIT_SIZE - 1 - j];
			}
		}
		state->u64[i] = temp;
		temp = 0;
...
/*
*	@brief Функция осуществляет генерацию раундового ключа
*/
static void generate_round_key(uint512_t* K, int round) 
{
	xor512(K, K, &gost3411_iter_consts[round]);
	sbox512(K);
	perm512(K);
	mul_matrix512(K);
}
...
/*
*	@brief Функция осуществляет преобразование E по ГОСТ-34.11
*/
static void compression_E(uint512_t* K, const uint512_t* m, uint512_t* state) 
{
	int i = 0;
	xor512(state, m, K);
	for (i = 0; i < COUNT_OF_ROUNDS; i++)
	{
		...
		perm512(state);
		mul_matrix512(state);
		generate_round_key(K, i);
		xor512(state, K, state);
	}
}

/*
*	@brief Функция осуществляет преобразование g по ГОСТ-34.11
*/
static void compression_G(uint512_t* h, uint512_t* N, const uint512_t* m) 
{
	uint512_t K = { 0 }, state = { 0 };
	xor512(&K, h, N);
	...
	compression_E(&K, m, &state);

	xor512(&state, h, &state);
	xor512(h, m, &state);
}

/*
*	@brief Функция осуществляет дополнение неполного блока нулями и 0x01 в конце
*/
static void padd_buffer(HashContext_t* ctx) 
{
	uint512_t temp = { 0 };
	if (ctx->tail_size < BLOCK_BYTE_SIZE) 
	{
		memset(temp.u8, 0x00, BLOCK_BYTE_SIZE);
		memcpy(temp.u8, ctx->buffer.u8, ctx->tail_size);
		temp.u8[ctx->tail_size] = 0x01;
		memcpy(ctx->buffer.u8, temp.u8, BLOCK_BYTE_SIZE);
		ctx->tail_size = BLOCK_BYTE_SIZE;
	}
}

/*
*	@brief Функция выполняет Этап 1 хэширования по ГОСТ-34.11
*/
static void stribog_stage_1(HashContext_t* ctx, const StribogMod_t mod)
{
	memset(ctx, 0x00, sizeof(*ctx));
	ctx->mod = mod;
	if (ctx->mod == STRIBOG256) 
	{
		memset(ctx->hash_current.u8, 0x01, BLOCK_BYTE_SIZE);
	}
	else 
	{
		memset(ctx->hash_current.u8, 0x00, BLOCK_BYTE_SIZE);
	}
	ctx->IV_512.u8[1] = 0x02; // IV_512 := 512
}

/*
*	@brief Функция выполняет Этап 2 хэширования по ГОСТ-34.11
*/
static void stribog_stage_2(HashContext_t* ctx, const uint512_t* data)
{
	compression_G(&ctx->hash_current, &ctx->N, data);
	add512(&ctx->N, &ctx->IV_512, &ctx->N);
	add512(&ctx->control_sum, data, &ctx->control_sum);
}

/*
*	@brief Функция выполняет Этап 3 хэширования по ГОСТ-34.11
*/
static void stribog_stage_3(HashContext_t* ctx)
{
	uint512_t temp = { 0 };
	temp.u8[1] = ((ctx->tail_size * 8) >> 8) & 0xff;
	temp.u8[0] = (ctx->tail_size * 8) & 0xff;

	padd_buffer(ctx);

	compression_G(&ctx->hash_current, &ctx->N, &ctx->buffer);

	add512(&ctx->N, &ctx->N, &temp);
	add512(&ctx->control_sum, &ctx->buffer, &ctx->control_sum);

	compression_G(&ctx->hash_current, &ctx->IV_0, &ctx->N);
	compression_G(&ctx->hash_current, &ctx->IV_0, &ctx->control_sum);

	memcpy(&ctx->hash_completed, &ctx->hash_current, BLOCK_BYTE_SIZE);
}
...
PUBLIC_FUNC status_t gost3411_HashInit(HashContext_t* ctx, StribogMod_t mod)
{
	...
	stribog_stage_1(ctx, mod);
	return STATUS_OK;
}

/*
*	@brief Функция запускает Этап 2 хэширования по ГОСТ-34.11 для буфера данных
*
*	@param[in, out] ctx - Указатель на контекст хэширования
...
PUBLIC_FUNC status_t gost3411_HashUpdate(HashContext_t* ctx, const uint8_t* data, size_t len)
{
	...
	uint512_t* buffer = (uint512_t*)data;
	while ((len > 63) && (ctx->tail_size) == 0) 
	{
		stribog_stage_2(ctx, (uint512_t*)data);
		data += BLOCK_BYTE_SIZE;
		len -= BLOCK_BYTE_SIZE;
	}
	...
	while (len)
	{
		chk_size = 64 - ctx->tail_size;
		if (chk_size > len)
		{
			chk_size = len;
		}
		memcpy(&ctx->buffer.u8[ctx->tail_size], data, chk_size);
		ctx->tail_size += chk_size;
		len -= chk_size;
		data += chk_size;
		if (ctx->tail_size == 64)
		{
			stribog_stage_2(ctx, &ctx->buffer);
			ctx->tail_size = 0;
		}
	}
	return STATUS_OK;
...
/*
*	@brief Функция осуществляет Этап 3 хэширования по ГОСТ-34.11
*
*	@param[in, out] ctx - Указатель на контекст хэширования
*/
